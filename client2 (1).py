...
file_t_location=os.path.expanduser('~')+'/Desktop/file_table1.pickle'
if not os.path.isfile(file_t_location):
	with open(file_t_location,'wb') as ftl:
		pickle.dump(dict(),ftl)
	fl=open(file_t_location,'rb')
...
def block_placement(i_file):
	...
	while n<len(ports):
		...
		port=ports[n]
		ip=ips[n]
		folder_name=''.join(i_file.split('/')[-1].split('.')[:-1])
		...
		# print('\n')
	with open('/home/aranya/Desktop/file_table1.pickle','wb') as out:
...
def clear_screen():
	if os.name=='nt':
		...
		_=os.system('clear')
def mr_service():
	os.system('python /home/aranya/Desktop/project/mr_service.py')
